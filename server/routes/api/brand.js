...
const validateBrandInput = (name, description) => {
  if (!description || !name) {
    throw { status: 400, message: 'You must enter description & name.' };
  }
};
...
router.get('/list', async (req, res) => {
  try {
    const brands = await Brand.find({ isActive: true }).populate('merchant', 'name');
...
router.get('/', auth, role.check(ROLES.Admin, ROLES.Merchant), async (req, res) => {
  try {
    ...
    if (req.user.merchant) {
      brands = await Brand.find({ merchant: req.user.merchant }).populate('merchant', 'name');
    } else {
...
router.put('/:id', auth, role.check(ROLES.Admin, ROLES.Merchant), async (req, res) => {
  try {
    ...
    const foundBrand = await Brand.findOne({ $or: [{ slug }] });

    if (foundBrand && foundBrand._id != brandId) {
      return res.status(400).json({ error: 'Slug is already in use.' });
    }

    await Brand.findOneAndUpdate(query, update, { new: true });
...
router.put('/:id/active', auth, role.check(ROLES.Admin, ROLES.Merchant), async (req, res) => {
  try {
    ...
    // Disable Brand's Products
    if (!update.isActive) {
      const products = await Product.find({ brand: brandId });
      store.disableProducts(products);
    }

    await updateBrandStatus(query, update);
...
// Helper Function: Deactivate Merchant
const deactivateMerchant = async (brandId) => {
  ...
  return await Merchant.findOneAndUpdate(query, update, { new: true });
};

// Helper Function: Update Brand's Status
const updateBrandStatus = async (query, update) => {
